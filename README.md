...
Each of these fixes patches a bug in someone else's mod. Once that mod's developer ships a real fix, the corresponding branch here is dead weight — delete the branch (`git push origin --delete <branch-name>`) and it's gone, with zero effect on any other fix.
